<template >
  <div style="padding-left:50px;padding-top:20px">
    <el-table :data="tableData" style="max-width: 1300px" v-loading="loading">
    <el-table-column label="名称" prop="filename" style="width: 200px">
      <template #default="scope">
        <span class="hoverFont" @click="openFile(scope.row.id, scope.row.filename, scope.row.fileType)">{{ scope.row.filename }}</span>
      </template>
    ...
    <el-table-column align="right">
      ...
      <template #default="scope">
        <el-button size="small" type="success" @click="downLoad(scope.row.id,scope.row.filename)">下载</el-button>
        <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" :icon="InfoFilled" icon-color="#626AEF"
          title="你确定要删除吗" @confirm="handleDelete(scope.row.id)">
          <template #reference>
  ...
  <el-pagination v-model:currentPage="pageQuery.pageNo" :page-sizes="[10, 30, 40, 50]" :page-size="pageQuery.pageSize"
    background="true" layout="sizes, total, prev, pager, next" :total="total" @size-change="handleSizeChange"
...
import { getFileList, deleteFile } from '../api/uploadList.js'
